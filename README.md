...
### The following is from PS course: ASP.NET Core Performance
There's a sample application that's referenced in some of the docs, and I'm showing the GitHub repo for it here. There's a great README. It overlaps heavily with some of the Microsoft docs on memory management, and it's worth your time to explore. I've got the repo cloned locally, and here it is in Visual Studio. There's an ApiController with a handful of methods that are meant to be called from some kind of load test, and we know how to do that now, right? There's a static string method that adds a bigString to a concurrent bag and returns that bigString. Then there's another method that just returns one. There's yet another method for getting a large byte array that you could use to explore the large object heap. And there are other methods that use file handles, HttpClients, and also some different ways of handling arrays. If we run the application, it's pretty neat. The main page for the application uses the diagnostics controller in the project to show us information similar to those memory tools in Visual Studio, but you can more clearly see the working set to the memory that's available to the app, as well as what's been allocated, and you can see that we're using the server mode for garbage collection. This continuously refreshes so you can explore some of the behaviors of different things on your own. And you can experiment by adding new controller actions and calling them. The README on this repo has some examples of what the graph looks like as load is pushed to it. It's a useful repo to be aware of, and, like the benchmarking technique that we saw earlier, could allow you to easily compare approaches from a memory standpoint.
